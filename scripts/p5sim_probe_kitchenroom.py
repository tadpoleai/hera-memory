...
print("Table049 prim valid:", prim.IsValid())
children = list(prim.GetChildren())
print("Table049 children:", [str(c.GetPath()) for c in children])

cache = UsdGeom.BBoxCache(Usd.TimeCode.Default(),
                          [UsdGeom.Tokens.default_, UsdGeom.Tokens.render])
rng = cache.ComputeWorldBound(prim).ComputeAlignedRange()
print("Table049 bbox min/max:", rng.GetMin(), rng.GetMax(), "empty:", rng.IsEmpty())
...
for p in stage.Traverse():
    n_prims += 1
    if p.GetTypeName() == "Xform" and not any(
            c.GetTypeName() == "Xform" for c in p.GetChildren()):
        n_leaf_xform += 1
...
print(f"leaf Xform: {n_leaf_xform}, with non-empty bbox: {n_leaf_nonempty}")
